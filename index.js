...
displayIcon.addEventListener('click', function onIconClicked(event) {
  if (event.target.matches('.showList')) {
    displayFormat = 'list'
    renderMovieList(getMoviesByPage(page))
  } else if (event.target.matches('.showCard')) {
    displayFormat = 'card'
    renderMovieList(getMoviesByPage(page))
  }
})
...
function renderMovieList(data) {
  ...
  if (displayFormat === 'card') {
    ...
    data.forEach((item) => {
      rawHTML += `<div class="listItem row" >
                      <div class="col-5"><h5>${item.title}</h5></div>
                      <div class="col-3"></div>
                      <div class="col-4">
                        <button class="btn btn-primary btn-show-movie" data-toggle="modal" data-target="#movie-modal" data-id="${item.id}">More</button>
                        <button class="btn btn-info btn-add-favorite" data-id="${item.id}">+</button>
